...
const THEMES = {
  bullish: { label: 'Opportunity', accent: '#00A37A', dark: '#04342C', strong: '#0F6E56', tagText: '#1D9E75', boxBg: '#ECFDF5', boxText: '#065F46', border: '#A7F3D0', readBg: '#E4F5EE', readStripe: '#9FE1CB', readTag: '#5DCAA5', readText: '#6E8079', readTime: '#9DB3AB' },
  bearish: { label: 'Caution', accent: '#D85A30', dark: '#4A1B0C', strong: '#993C1D', tagText: '#D85A30', boxBg: '#FAECE7', boxText: '#993C1D', border: '#F5C4B3', readBg: '#F7EAE3', readStripe: '#F0997B', readTag: '#D89177', readText: '#8A7064', readTime: '#B5A296' },
  neutral: { label: 'Watch', accent: '#BA7517', dark: '#412402', strong: '#854F0B', tagText: '#BA7517', boxBg: '#FAEEDA', boxText: '#854F0B', border: '#FAC775', readBg: '#F6EEDD', readStripe: '#FAC775', readTag: '#C9A86A', readText: '#8A7B5E', readTime: '#B5A88A' },
};
...
const TAGLINES = ['News that moves your money — before the market reacts.', 'Your edge in the market starts here.', 'Trade the news. Beat the crowd.', 'Real news. Real impact. Real fast.', 'For traders who move before the herd.'];
...
export default function Home() {
  ...
  const filtered = cards.filter((c) => {
    const searchOk = !searchQuery || (c.ticker || '').toLowerCase().includes(searchQuery.toLowerCase()) || (c.title || '').toLowerCase().includes(searchQuery.toLowerCase()) || (c.sector || '').toLowerCase().includes(searchQuery.toLowerCase());
  ...
  useEffect(() => {
    const check = async () => {
      try {
        ...
        if (fresh.length > 0) { setPendingCards(data); setNewCount(fresh.length); }
      } catch (e) { }
    };
  ...
  const hotTopics = [...cards].sort((a, b) => (b.confidence || 0) - (a.confidence || 0)).slice(0, 4);
  ...
  const moodFor = (days) => {
    ...
    return { bull: Math.round((bull / total) * 100), bear: Math.round((bear / total) * 100), neut: Math.round((neut / total) * 100), total };
  };
  const moodTimeframes = [{ label: 'Daily', m: moodFor(1) }, { label: 'Weekly', m: moodFor(7) }];
    ...
    <div style={s.page}>
      ...
      <section style={s.hero}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          {newCount > 0 && (
            <button onClick={() => { if (pendingCards) { setCards(pendingCards); setPendingCards(null); setNewCount(0); setPage(0); } }} style={s.newPill}>
        ...
        <aside className="sai-left" style={s.left}>
          ...
          <div className="sai-filter-list" style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {filters.map((f) => {
              ...
              return (
                <button key={f} onClick={() => setSentiment(f)} style={{ ...s.filterBtn, background: isActive ? t.accent : '#FFFFFF', color: isActive ? '#FFFFFF' : t.tagText, fontWeight: isActive ? 700 : 500, border: isActive ? 'none' : '1px solid #E4DCCE' }}>
        ...
        <main style={s.center}>
          <div style={s.gridHeader}>
            ...
            <div style={s.filterAnalystRow}>
              ...
              <label style={s.analystLabel}>
                <input type="checkbox" checked={analystOnly} onChange={(e) => setAnalystOnly(e.target.checked)} />
                <span>⭐ Picks</span>
              </label>
            </div>
            ...
            <div style={s.searchSectorRow}>
              ...
              <select value={sectorFilter} onChange={(e) => setSectorFilter(e.target.value)} style={s.sectorSelect}>
                {sectorOptions.map((sec) => (<option key={sec} value={sec}>{sec === 'All' ? 'All sectors' : sec}</option>))}
              </select>
            </div>
          </div>
          ...
          {!loading && !error && visibleCards.length === 0 && (<div style={s.muted}>No news in this filter right now. Check back soon.</div>)}
          ...
          {!loading && visibleCards.length > 0 && (
            <div className="sai-grid" style={s.grid}>
              {visibleCards.map((card, idx) => {
                ...
                return (
                  <button key={card.id} onClick={() => openCard(idx)} style={{ ...s.tile, background: isRead ? t.readBg : '#FFFFFF', borderLeft: `3px solid ${isRead ? t.readStripe : t.accent}`, boxShadow: isRead ? 'none' : '0 1px 3px rgba(0,0,0,0.05)' }}>
                    <div>
                      <span style={{ ...s.tileTag, color: isRead ? t.readTag : t.tagText }}>{card.sentiment || 'Neutral'}{card.sector ? ` · ${card.sector}` : ''}{isRead ? ' · ✓ Read' : ''}</span>
          ...
          {!loading && filtered.length > PAGE_SIZE && (
            <div style={s.pagination}>
              <button onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={safePage === 0} style={{ ...s.pageBtn, ...(safePage === 0 ? s.pageBtnDisabled : s.pageBtnActive) }}>‹ Prev</button>
              <span style={s.pageLabel}>Page {safePage + 1} of {totalPages}</span>
              <button onClick={() => setPage((p) => Math.min(totalPages - 1, p + 1))} disabled={safePage >= totalPages - 1} style={{ ...s.pageBtn, ...(safePage >= totalPages - 1 ? s.pageBtnDisabled : s.pageBtnActive) }}>Next ›</button>
            </div>
          )}
        </main>

        <aside className="sai-right" style={s.right}>
          <div style={{ ...s.moodBox, borderColor: moodOpen ? '#BFD9D3' : '#ECE4D6' }} onMouseEnter={() => setMoodOpen(true)} onMouseLeave={() => setMoodOpen(false)}>
      ...
      {activeIndex !== null && visibleCards[activeIndex] && (
        <FullCard card={visibleCards[activeIndex]} onClose={() => setActiveIndex(null)} onPrev={activeIndex > 0 ? () => setActiveIndex(activeIndex - 1) : null} onNext={activeIndex < visibleCards.length - 1 ? () => setActiveIndex(activeIndex + 1) : null} current={activeIndex + 1} total={visibleCards.length} />
      )}
      ...
      <footer style={s.footer}>
        ...
        <p style={s.disclaimer}>Financial news and information for educational purposes only. Nothing here is investment advice. Always do your own research.</p>
      </footer>
      ...
      <style>{`
        ...
        @media (max-width: 860px) {
          ...
          .sai-left { display: none !important; }
        }
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ ...s.modal, border: `1px solid ${t.border}` }}>
        ...
        <div style={s.modalBody}>
          ...
          <div style={s.modalFooter}>
            <span>🕐 {published}</span>
            {card.url && (<a href={card.url} target="_blank" rel="noopener noreferrer" style={s.sourceLink}>{card.source || 'Source'} ↗</a>)}
          </div>
        </div>

        <div style={s.navBar}>
          <button onClick={(e) => { e.stopPropagation(); onPrev && onPrev(); }} disabled={!onPrev} style={{ ...s.navBtn, opacity: onPrev ? 1 : 0.3, cursor: onPrev ? 'pointer' : 'default' }}>‹ Previous</button>
          <button onClick={(e) => { e.stopPropagation(); onNext && onNext(); }} disabled={!onNext} style={{ ...s.navBtn, opacity: onNext ? 1 : 0.3, cursor: onNext ? 'pointer' : 'default' }}>Next ›</button>
        </div>
      </div>
    </div>
  );
...
const s = {
  ...
  hero: { background: 'linear-gradient(135deg, #0D5C6E 0%, #0B3038 100%)', padding: '18px 28px 32px', position: 'relative', overflow: 'hidden', margin: 16, borderRadius: 16 },
  ...
  newPill: { display: 'inline-flex', alignItems: 'center', gap: 8, marginTop: 16, background: '#00C896', border: 'none', padding: '8px 15px', borderRadius: 20, cursor: 'pointer', fontFamily: FONT, fontSize: 12, color: '#04342C', fontWeight: 700, boxShadow: '0 2px 10px rgba(0,200,150,0.35)' },
  ...
  filterAnalystRow: { display: 'flex', gap: 8, alignItems: 'center' },
  filterSelect: { flex: 1, padding: '8px 10px', fontSize: 12, border: '1px solid #E4DCCE', borderRadius: 8, background: '#fff', fontFamily: FONT, fontWeight: 600, cursor: 'pointer', outline: 'none' },
  analystLabel: { display: 'flex', alignItems: 'center', gap: 6, padding: '8px 10px', background: '#fff', border: '1px solid #E4DCCE', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap' },
  ...
  tile: { textAlign: 'left', borderRadius: 12, padding: 13, minHeight: 100, cursor: 'pointer', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', border: 'none', fontFamily: FONT, borderLeftWidth: 3, borderLeftStyle: 'solid', transition: 'all 0.15s ease' },
  ...
  hotItem: { textAlign: 'left', background: '#FFFFFF', borderRadius: 10, padding: 10, border: '1px solid #ECE4D6', cursor: 'pointer', fontFamily: FONT },
  ...
  overlay: { position: 'fixed', inset: 0, background: 'rgba(40,30,20,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, zIndex: 1000 },
  modal: { background: '#FFFDF9', borderRadius: 16, maxWidth: 420, width: '100%', maxHeight: '90vh', position: 'relative', fontFamily: FONT, display: 'flex', flexDirection: 'column', overflow: 'hidden' },
  modalTop: { padding: '16px 22px 0', flexShrink: 0 },
  modalBody: { padding: '4px 22px', overflowY: 'auto', flex: 1 },
  close: { position: 'absolute', top: 12, right: 12, background: 'rgba(0,0,0,0.06)', border: 'none', width: 28, height: 28, borderRadius: '50%', cursor: 'pointer', fontSize: 13, color: '#444', zIndex: 2 },
  ...
  modalFooter: { paddingTop: 10, paddingBottom: 4, borderTop: '0.5px solid #ECE4D6', display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 12, color: '#A89A82' },
  sourceLink: { color: '#0D5C6E', textDecoration: 'none', fontWeight: 700 },
  navBar: { display: 'flex', alignItems: 'center', gap: 10, padding: '12px 22px 18px', flexShrink: 0, borderTop: '0.5px solid #ECE4D6', background: '#FFFDF9' },
  navBtn: { flex: 1, background: '#0D5C6E', color: '#fff', border: 'none', padding: '12px 16px', borderRadius: 10, fontSize: 14, fontWeight: 700, fontFamily: FONT },
  ...
  footer: { background: '#0D5C6E', color: '#C8E9DF', padding: '28px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, fontSize: 13 },
